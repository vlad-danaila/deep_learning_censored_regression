...
train_and_evaluate_net_mae = train_and_evaluate_mae_mse(ROOT_MSE + '/' + CHECKPOINT_MSE,
...
def grid_search_mse_simple():
    ...
    grid_best = grid_search(ROOT_MSE, dataset_train, dataset_val, bound_min, bound_max,
            grid_config, train_and_evaluate_net_mae, CHECKPOINT_MSE, conf_validation = config_validation)
    return grid_best
...
def plot_mse_simple():
    checkpoint = load_checkpoint(f'{ROOT_MSE}/grid {CHECKPOINT_MSE}.tar')
...
train_and_evaluate_net_mae_cens = train_and_evaluate_mae_mse(ROOT_BOUNDED_MSE + '/' + CHECKPOINT_BOUNDED_MSE,
...
def grid_search_mse_cens_NO_trunc():
    ...
    grid_best = grid_search(ROOT_BOUNDED_MSE, dataset_train, dataset_val, bound_min, bound_max,
        grid_config, train_and_evaluate_net_mae_cens, CHECKPOINT_BOUNDED_MSE, conf_validation = config_validation)
    return grid_best
...
def plot_mse_cens_NO_trunc():
    checkpoint = load_checkpoint(f'{ROOT_BOUNDED_MSE}/grid {CHECKPOINT_BOUNDED_MSE}.tar')
    plot_dataset_and_net(checkpoint, get_model(INPUT_SIZE), test_df(df))
...
train_and_evaluate_net_mae_cens_trunc = train_and_evaluate_mae_mse(ROOT_BOUNDED_MSE_WITH_PENALTY + '/' + CHECKPOINT_BOUNDED_MSE_WITH_PENALTY,
...
def grid_search_mse_cens_WITH_trunc():
    ...
    grid_best = grid_search(ROOT_BOUNDED_MSE_WITH_PENALTY, dataset_train, dataset_val, bound_min, bound_max,
        grid_config, train_and_evaluate_net_mae_cens_trunc, CHECKPOINT_BOUNDED_MSE_WITH_PENALTY, conf_validation = config_validation)
    return grid_best
...
def plot_mse_cens_WITH_trunc():
    checkpoint = load_checkpoint(f'{ROOT_BOUNDED_MSE_WITH_PENALTY}/grid {CHECKPOINT_BOUNDED_MSE_WITH_PENALTY}.tar')
    plot_dataset_and_net(checkpoint, get_model(INPUT_SIZE), test_df(df))