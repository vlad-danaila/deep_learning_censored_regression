...
def censored_mae(y_pred, y, lower_censoring_bound = -math.inf, upper_censoring_bound = math.inf):
  y_pred = t.where((y > lower_censoring_bound) + (y_pred > lower_censoring_bound), y_pred, t.tensor(lower_censoring_bound))
  y_pred = t.where((y < upper_censoring_bound) + (y_pred < upper_censoring_bound), y_pred, t.tensor(upper_censoring_bound))
  return mae(y_pred, y)
