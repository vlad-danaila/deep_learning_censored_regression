...
def censored_mae(y_pred, y, lower_censoring_bound = -math.inf, upper_censoring_bound = math.inf, device = 'cpu', clamp_all = True):
  if clamp_all:
...
def truncation_mse_penalty(y_pred, lower_truncation_limit = None, upper_truncation_limit = None):
  lower_truncation_penalty, upper_truncation_penalty = 0, 0
  if lower_truncation_limit:
    below_lower_truncation_limit = t.clamp(y_pred, min = -math.inf, max = lower_truncation_limit)
    lower_truncation_penalty = mse(below_lower_truncation_limit, t.full_like(below_lower_truncation_limit, lower_truncation_limit))
  if upper_truncation_limit:
    above_upper_truncation_limit = t.clamp(y_pred, min = upper_truncation_limit, max = math.inf)
...
def censored_mse_with_truncation_penalty(
        y_pred,
        y,
        lower_censoring_bound = -math.inf,
        upper_censoring_bound = math.inf,
