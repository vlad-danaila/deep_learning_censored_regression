...
class TobitOptimizationTest(unittest.TestCase):
    ...
    def test_uncensored(self):
        ...
        loss = censored_mse_with_truncation_penalty(y_pred = y_pred, y = y, clamp_all = False)
    ...
    def test_lower_censored_1(self):
        ...
        loss = censored_mse_with_truncation_penalty(y_pred = y_pred, y = y, lower_censoring_bound = low_censor, clamp_all = False)
        self.assertTrue(t.allclose(loss, expected_loss))
    ...
    def test_lower_censored_3(self):
        ...
        expected_loss = t.tensor((.49 + .09)/3)
        loss = censored_mse_with_truncation_penalty(y_pred = y_pred, y = y, lower_censoring_bound = low_censor, clamp_all = False)
        self.assertTrue(t.allclose(loss, expected_loss))
    ...
    def test_upper_censored_1(self):
        ...
        y_pred = t.tensor([1.3, 1.7, 2.3])
        expected_loss = t.tensor((.01 + .04)/3)
        loss = censored_mse_with_truncation_penalty(y_pred = y_pred, y = y, upper_censoring_bound = upper_censor, clamp_all = False)
        self.assertTrue(t.allclose(loss, expected_loss))
    ...
    def test_upper_censored_3(self):
        ...
        expected_loss = t.tensor((.01 + .25)/3)
        loss = censored_mse_with_truncation_penalty(y_pred = y_pred, y = y, upper_censoring_bound = upper_censor, clamp_all = False)
        self.assertTrue(t.allclose(loss, expected_loss))
    ...
    def test_combined_1(self):
        ...
        expected_loss = t.tensor((0 + .01 + .04 + .25 + .01 + 0)/6)
        loss = censored_mse_with_truncation_penalty(y_pred = y_pred, y = y, lower_censoring_bound = lower_censor, upper_censoring_bound = upper_censor, clamp_all = False)
        self.assertTrue(t.allclose(loss, expected_loss))
    ...
    def test_truncated_1(self):
        ...
        expected_loss = t.tensor((0.04 + 0.01 + 0.01 + 0)/3)
        loss = censored_mse_with_truncation_penalty(y_pred=y_pred, y=y, lower_truncation_limit = lower_truncation, clamp_all = False)
        self.assertTrue(t.allclose(loss, expected_loss))
    ...
    def test_truncated_2(self):
        ...
        expected_loss = t.tensor((0.04 + 0.01 + 0.09 + 0.01 + 0)/3)
        loss = censored_mse_with_truncation_penalty(y_pred=y_pred, y=y, lower_truncation_limit = lower_truncation, clamp_all = False)
        self.assertTrue(t.allclose(loss, expected_loss))
    ...
    def test_truncated_3(self):
        ...
        loss = censored_mse_with_truncation_penalty(y_pred=y_pred, y=y, upper_truncation_limit = upper_truncation, clamp_all = False)
        self.assertTrue(t.allclose(loss, expected_loss))
    ...
    def test_combined_2(self):
        ...
        expected_loss = t.tensor((.01 + .01 + .04 + .25 + 0.04 + .01 + .01)/6)
        loss = censored_mse_with_truncation_penalty(y_pred = y_pred, y = y,
            lower_censoring_bound = lower_censor, upper_censoring_bound = upper_censor, lower_truncation_limit = lower_truncation, upper_truncation_limit = upper_truncation, clamp_all = False)
        self.assertTrue(t.allclose(loss, expected_loss))